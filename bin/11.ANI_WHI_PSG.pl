...
close IF;
print OUT "Ref\tQuery\tRef_Size\tQuery_Size\tANI\tWHI\tPSG\n";
while(<LIST>){
	chomp;
	my $file=$_;
	...
	if($sumlen == 0){
		...
		my $PSG=sprintf("%.2f",($len1+$len2)*100/($totallen1+$totallen2));
		my $WHI=sprintf("%.2f",$p*($len1+$len2)*100/($totallen1+$totallen2));
